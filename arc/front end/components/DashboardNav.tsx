...
function NavLinks({ nav, path, onNavigate, mobile = false }: { nav: readonly DashboardNavItem[]; path: string; onNavigate?: () => void; mobile?: boolean }) {
  return <>
    {nav.map(([href, Icon, label]) => <Link key={href} to={href} onClick={onNavigate} className={`inline-flex shrink-0 items-center gap-2 rounded-[5px] border px-4 py-3 ${mobile ? "text-[18px]" : "text-[15px]"} font-bold transition-colors ${mobile ? (path === href ? "border-[#f5b544] bg-[#f5b544] text-black" : "border-white/25 bg-transparent text-white hover:border-[#f5b544] hover:text-[#f5b544]") : (path === href ? "border-black bg-black text-white" : "border-gray-400 bg-white text-black hover:bg-black hover:text-white")}`}><Icon className="h-4 w-4" />{label}</Link>)}
  </>;
}
...
export default function DashboardNav({ path, roleLabel, workspaceTitle, nav }: DashboardNavProps) {
  ...
  return <header className="relative z-40 border-b border-gray-400 bg-white text-black shadow-sm">
    <div className="mx-auto flex h-[74px] max-w-[1240px] items-center justify-between gap-4 px-5 lg:px-8">
      ...
      <DashboardAvatar />
      <button type="button" aria-label={menuOpen ? "Close dashboard navigation" : "Open dashboard navigation"} aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)} className="rounded-[5px] border border-gray-400 p-3 transition-transform hover:bg-black hover:text-white lg:hidden">
    ...
    {menuOpen && <div className="fixed left-0 top-[74px] z-50 h-[calc(100vh-74px)] w-[min(88vw,380px)] overflow-y-auto border-r border-white/20 bg-black p-5 text-white shadow-2xl lg:hidden">
      <p className="text-[18px] font-extrabold">{workspaceTitle}</p>
      ...
      <Link to="/" onClick={() => setMenuOpen(false)} className="mt-8 inline-flex items-center rounded-[5px] border border-white/40 px-4 py-3 text-[15px] font-bold text-white">Exit workspace</Link>
    </div>}
  </header>;
}
